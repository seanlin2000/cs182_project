...
def main():
    ...
    model_test = Ensemble(device)
    print(device)
    
    model_test = model_test.to(device)
    model_solver = NickFury("ensemble", model_test, dataloaders, dataset_sizes, device)
    
    print(model_solver.accuracy('val'))
    print(model_solver.top_k_accuracy(5, 'val'))
    torch.save({
        'overnight': model_test.state_dict(),
    }, 'ensemble.pt')
